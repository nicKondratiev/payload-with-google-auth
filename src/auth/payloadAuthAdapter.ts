...
import { type User, type Account } from '@/payload-types'
...
const covertPayloadUserToAdapterUser = (user: User): any => {
  return {
    ...user,
    id: String(user.id),
  ...
  return {
    createUser: async ({ id, ...data }) => {
      ...
      if (adminEmails.includes(data.email)) {
        role = 'admin'
      } else if (editorEmails.includes(data.email)) {
        role = 'editor'
      } else {
        return null
        // throw new Error(`Unauthorized email: ${data.email}`)
      }
